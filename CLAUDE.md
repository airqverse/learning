...
| `google_cloud/` | Diary-driven GCP learning — daily diaries in `01_Diaries/` get AI-extracted into flashcards in `98_Flashcards/` for Google Cloud certification prep |
...
- **Concepts vs. Implementation must never be mixed.** In DSA: algorithm theory lives in `02_Algorithms/`, LeetCode solutions live in `05_Problems/`. Cross-link between them.
